...
def compute_TCA(X1, X2, trange, rso1_params, rso2_params, int_params,
    ...
    minimum of the relative distance between orbits. The function will either
    ...
    while b <= trange[1]:
        ...
        # print(f"vcomute_TCA: {rso1_params}, {rso2_params}")
...
def gvec_tudat(t0, tvec, X1, X2, rso1_params, rso2_params, int_params, bodies):
    ...
    for ti in tvec:
        ...
        if ti == t0:
            ...
            tin = [t0, ti]
            # tin = [tvec[0], ti]
            # print(f"vcomute_gvec: {rso1_params}, {rso2_params}")
...
def compute_miss_distance(X1, X2):
    """
    Computes the Euclidean miss distance between two position vectors, given the state vectors.
    """
    rA = X1[:3]
    rB = X2[:3]
    delta_r = rA - rB
    d2 = np.sqrt(np.dot(delta_r.T, delta_r))
    return float(d2)

def compute_mahalanobis_distance(X1, X2, P1, P2):
    """
    Computes the Mahalanobis miss distance between two position vectors
    considering their covariance matrices.
    """
    rA = X1[:3]
    rB = X2[:3]
    PA = P1[:3,:3]
    PB = P2[:3,:3]
    delta_r = rA - rB
    combined_covariance = PA + PB
    try:
        inv_covariance = np.linalg.inv(combined_covariance)
    except np.linalg.LinAlgError:
        # Handle singular matrix
        print("Warning: Singular covariance matrix, using pseudo-inverse.")
        inv_covariance = np.linalg.pinv(combined_covariance)
    dM = np.sqrt(np.dot(delta_r.T, np.dot(inv_covariance, delta_r)))
    return float(dM)

# def relative_position(position1, position2):
#     """
#     Calculate the relative position between two objects.
#     """
...
#     """
#     Calculate the relative speed between two objects.
#     """
#     #todo: complete this function
#     return np.linalg.norm(velocity1 - velocity2)

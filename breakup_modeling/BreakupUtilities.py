...
def plot_elements(tout, yout):
    ...
    for ii in range(len(tout)):
        if yout[ii,2] < 0:
            yout[ii,2] = math.fmod(yout[ii,2], np.pi) + np.pi
        
        if yout[ii,2] > np.pi:
            yout[ii,2] = math.fmod(yout[ii,2], np.pi)
        
        if yout[ii,3] < 0:
            yout[ii,3] = math.fmod(yout[ii,3], 2*np.pi) + 2*np.pi
        
        if yout[ii,3] > 2*np.pi:
            yout[ii,3] = math.fmod(yout[ii,3], 2*np.pi)
        
        if yout[ii,4] < 0:
            yout[ii,4] = math.fmod(yout[ii,4], 2*np.pi) + 2*np.pi
        
        if yout[ii,4] > 2*np.pi:
            yout[ii,4] = math.fmod(yout[ii,4], 2*np.pi)
            
    
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(t_yrs, yout[:,0]/1000., 'k.')
    plt.ylabel('SMA [km]')
    plt.ylim([min(yout[:,0]/1000. - 10.), max(yout[:,0]/1000. + 10.)])
    plt.subplot(2,1,2)
    ...
    plt.figure()
    plt.subplot(3,1,1)
    plt.plot(t_yrs, yout[:,2]*180/np.pi, 'k.')
    plt.ylabel('Inc [deg]')
    plt.ylim([0., 180.])
    plt.subplot(3,1,2)
    plt.plot(t_yrs, yout[:,3]*180/np.pi, 'k.')
    plt.ylabel('RAAN [deg]')
    plt.ylim([-1., 400.])
    plt.subplot(3,1,3)
    plt.plot(t_yrs, yout[:,4]*180/np.pi, 'k.')
    plt.ylim([-1, 400.])
    plt.ylabel('AoP [deg]')
    plt.xlabel('Time [years]')
...
def plot_longitude(tout, yout):
    
    t_yrs = [(ti - tout[0])/(365.25*86400) for ti in tout]
    longitude = yout[:,-1]*180/np.pi

    plt.figure()
    plt.plot(t_yrs, longitude)
    plt.xlabel('Time [years]')
...
def plot_breakup_stats(lc_array, N_bin, N_cum, lc_list_full, A_list, A_m_list, m_list, dV_list):
    ...
    plt.figure()
    plt.loglog(lc_array, N_cum, 'k.')
    plt.xlabel('Diameter [m]')
    plt.ylabel('Cumulative Number of Objects')

    plt.figure()
    plt.loglog(lc_array, N_bin, 'k.')
    plt.xlabel('Diameter [m]')
    plt.ylabel('Number of Objects Per Bin')
...
def plot_gabbard(kep_array, GM=GME, R=Re):
    '''
    This function generates a gabbard plot for an input matrix of state
     vectors in either Cartesian position/velocity or Keplerian elements.
     
     Parameters
     ------
     kep_array : Nx6 array
       each row corresponds to an object state vector in Keplerian orbital 
     ...
     Returns
     ------
     Gabbard plot of apoapsis/periapsis altitude vs orbit period
    ...
    hp_list = []
    P_list = []
    for ii in range(kep_array.shape[0]):
        
        elem = kep_array[ii,:].flatten()
        ...
        ha = ra - R
        hp = rp - R
        P = 2*np.pi*np.sqrt(a**3./GM) * 1/60.
        
        if hp > 100000.:
        
            ha_list.append(ha/1000.)
            hp_list.append(hp/1000.)
            P_list.append(P)
            
        
    plt.figure()
    plt.plot(P_list, ha_list, 'b.')
    plt.plot(P_list, hp_list, 'r.')
...
def plot_inc_raan(kep_array, GM=GME, R=Re):
    '''
    This function generates a plot of inclination vs RAAN for an input matrix 
    of state vectors in either Cartesian position/velocity or Keplerian 
    elements.
    ...
    inc_deg = kep_array[:,2]*180./np.pi
    raan_deg = kep_array[:,3]*180./np.pi
    raan_deg = np.mod(raan_deg, 360.)
    
    plt.figure()
    plt.plot(raan_deg, inc_deg, 'k.')
    plt.xlabel('RAAN [deg]')
    plt.ylabel('Inclination [deg]')
    plt.xlim([0., 360.])
    plt.ylim([min(inc_deg)-5., max(inc_deg)+5.])
    
    
    return


def plot_latlon(latlon_array):
    
    
    lat = latlon_array[:,0]*180./np.pi
    lon = latlon_array[:,1]*180./np.pi
...
def plot_reentry_stats(reentry_times, nobj, tin):
    

    reentry_times = sorted(reentry_times)    
    nobj_list = [nobj]
    for ii in range(len(reentry_times)):
        nobj_list.append(nobj_list[ii]-1)
        
    # print(reentry_times)

    nobj_list.append(nobj_list[-1])
    t_list = [0.]
    t_list.extend(reentry_times)
    t_list.append(tin[1]-tin[0])
    
    t_list = [ti/(86400.*365.25) for ti in t_list]
    percent_list = [ni/nobj_list[0]*100. for ni in nobj_list]
    
    plt.figure()
    plt.plot(t_list, percent_list, 'k.-')
    plt.ylabel('Percent of Objects on Orbit')
    plt.xlabel('Time [years]')

...
def gaussian_probabilities():
    ...
    mean = np.array([lam])
    P = np.array([[sig**2.]])
    samples = np.random.default_rng().multivariate_normal(mean, P, N)
    ...
    print('1D Gaussian 1-sig', P_1sig)
    print('1D Gaussian 2-sig', P_2sig)
    print('1D Gaussian 3-sig', P_3sig)
    ...
    c = 0.
    for ii in range(N):
        x = samples[ii,:].reshape(2,1)
        
        d_M = float(np.sqrt(np.dot(x.T, np.dot(invP, x)))[0,0])
    ...
    print('2D Gaussian 1-sig', P_1sig)
    print('2D Gaussian 2-sig', P_2sig)
    print('2D Gaussian 3-sig', P_3sig)
    ...
    print('3D Gaussian 1-sig', P_1sig)
    print('3D Gaussian 2-sig', P_2sig)
    print('3D Gaussian 3-sig', P_3sig)
    print('3D Gaussian 10-sig', P_10sig)
